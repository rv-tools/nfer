...
/*
The greedy optimization consumes intervals when they are used to create new intervals,
so any interval may only be used to create one new interval per rule.
 */
...
private[nfer] class Node {
  ...
  var usedTokens: Map[Token,(Token,Token)] = Map() // NEW - TODO: implement New as set of triples (see algo)
  val emptyTokenSet: Set[Token] = Set()
  val emptyData: Data = Map()
  val emptyEnv: Map[String,Interval] = Map() // NEW
  val emptyUsedTokenMap: Map[Token,(Token,Token)] = Map() // NEW
  ...
  def addToken(token: Token, inputNode: Node): Unit = {
    if (inputNode.eq(inputNode1)) {
      ...
      // inputNode == inputNode2 && inputNode1 == null: atomic child
      tokensNew += token
    }
    for (token <- tokensNew) evaluateToken(token)
    tokensNew = emptyTokenSet
    usedTokens = emptyUsedTokenMap // NEW
  }
  ...
  def applyTemporalOperator(token1: Token, token2: Token) {
    ...
    if (predefinedOrdered(interval1, interval2)) {
      ...
      tokensNew += newToken
      usedTokens += (newToken -> (token1, token2)) // NEW
    }
  }

  def evaluateToken(token: Token): Unit = {
    var added: Boolean = false // NEW
    if (minimal(token)) {
      if (outputNode != null) {
        added = true // NEW
        tokens += token
        ...
        if (evaluateConstraint(token)) {
          ...
          val newToken = Token(newInterval, token.environment) // NEW
          tokens += newToken
    ...
    if (added) {
      // NEW
      usedTokens.getOrElse(token,null) match {
        case null =>
        case (left, right) =>
          // inputNode1.tokens = inputNode1.tokens.filterNot(_ eq left)
          // inputNode2.tokens = inputNode2.tokens.filterNot(_ eq right)
          inputNode1.tokens = inputNode1.tokens - left
          inputNode2.tokens = inputNode2.tokens - right
      }
    }
  }
  ...
  def minimalIn(token: Token, tokenSet: Set[Token]): Boolean =
    !(tokenSet exists (smallerToken => token.interval.contains(smallerToken.interval)))
